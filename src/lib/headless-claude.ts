...
function isoStampForLog(d: Date): string {
  return d.toISOString().replace(/[:.]/g, '-');
}

function resolveTimeout(opts: FireOptions): number {
...
function resolveClaudeBin(opts: FireOptions): string {
  const env = opts.env ?? process.env;
...
export function parseEnvelope(stdout: string): {
  ...
  if (!last) return { error: 'empty stdout' };
  try {
    const parsed = JSON.parse(last);
    if (parsed === null || typeof parsed !== 'object') {
      return { error: 'envelope was not a JSON object' };
    }
    return { envelope: parsed as ClaudeResultEnvelope };
  } catch (err) {
    return { error: `envelope JSON.parse failed: ${(err as Error).message}` };
  }
}

async function tryWriteFile(p: string, body: string): Promise<void> {
  try {
    ...
    console.error(`[headless-claude] failed to write ${p}: ${(err as Error).message}`);
  }
}

export async function fireHeadlessSession(opts: FireOptions): Promise<FireResult> {
  ...
  const logBody =
    `# session-orchestrator phase ${opts.phase} run log\n` +
  ...
  const finalize = async (r: FireResult): Promise<FireResult> => {
    await tryWriteFile(resultJsonPath, JSON.stringify(r, null, 2));
    const runUrl = (opts.env ?? process.env).GITHUB_RUN_URL || undefined;
    await tryWriteFile(
      digestPath,
      renderDigest({ result: r, config: opts.config, phase: opts.phase, runUrl }),
    );
    return r;
  };
  ...
  if (spawnError) {
    await tryWriteFile(
      failedMarkerPath,
      `phase ${opts.phase} spawn failure at ${finishedAt.toISOString()}\n` +
        `${spawnError}\nlog: ${logPath}\n`,
    );
    return finalize({
      kind: 'failure',
  ...
  if (timedOut) {
    await tryWriteFile(
      failedMarkerPath,
      `phase ${opts.phase} timed out at ${finishedAt.toISOString()} ` +
        `after ${timeoutMs}ms\nlog: ${logPath}\n`,
    );
    return finalize({
      kind: 'failure',
  ...
  if (exitCode !== 0) {
    await tryWriteFile(
      failedMarkerPath,
      `phase ${opts.phase} exited ${exitCode} at ${finishedAt.toISOString()}\n` +
  ...
  if (!envelope) {
    await tryWriteFile(
      failedMarkerPath,
      `phase ${opts.phase} succeeded on exit but envelope unparseable: ${envelopeParseError}\nlog: ${logPath}\n`,
    );
    return finalize({
      kind: 'failure',
  ...
  if (envelope.is_error === true) {
    await tryWriteFile(
      failedMarkerPath,
      `phase ${opts.phase} envelope is_error=true at ${finishedAt.toISOString()}\n` +
