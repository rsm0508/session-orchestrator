...
export default class Resume extends Command {
  static description =
    'Remove the repo-level kill-switch file. Note: env var and Linear-label paths must be cleared separately.';
  ...
  async run(): Promise<void> {
    ...
    this.log(
      `Reminder: env var ${PAUSE_ENV_VAR} and Linear label "${LINEAR_PAUSE_LABEL}" can also halt runs — clear those separately if set.`,
    );
  }
}
