...
export function handoffPathFor(config, phase) {
    return config.handoff_pattern.replace('{N}', String(phase));
}
export function startedMarkerRelativePath(phase) {
    return path.posix.join(MARKERS_DIR, `phase-${phase}.started`);
}
export function failedMarkerRelativePath(phase) {
...
export async function scanPhases(repoRoot, config) {
    const phases = [];
 ...
 */
export async function resolveNextPhase(repoRoot, config) {
    ...
    if (failedBlocked) {
        return {
            ...
            details: `Phase ${failedBlocked.phase} has a .failed marker at ${failedBlocked.failedMarkerPath} — ` +
    ...
    const someHandoffMissing = phases.some((p) => !p.handoffExists && !p.startedMarkerExists && !p.failedMarkerExists);
    if (someHandoffMissing) {
        const first = phases.find((p) => !p.handoffExists && !p.startedMarkerExists && !p.failedMarkerExists);
        return {
            kind: 'not-ready',
