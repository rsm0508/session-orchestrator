...
export default class Run extends Command {
    ...
    async run() {
        ...
        if (markerExists || failedExists) {
            ...
            const which = [];
            if (markerExists)
                which.push(startedRel);
            if (failedExists)
                which.push(failedRel);
            this.error(`Phase ${flags.phase} is already in a marker state: ${which.join(', ')}. ` +
        ...
        if (!process.env.ANTHROPIC_API_KEY) {
            this.error('ANTHROPIC_API_KEY env var is not set. The headless `claude --bare` ' +
                'invocation requires it (OAuth and keychain are NOT read in --bare mode). ' +
                'In GitHub Actions, pass it via `env: { ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }} }`.', { exit: 2 });
        }
        ...
        if (result.kind === 'success') {
            this.log(`[fire] OK — exit 0, duration ${result.durationMs}ms, ` +
                `turns=${result.envelope.num_turns ?? '?'}, cost=$${(result.envelope.total_cost_usd ?? 0).toFixed(4)}`);
        ...
        if (result.reason === 'marker-collision') {
            ...
            this.log('  If the prior fire crashed and the marker is stale, delete ' +
        ...
        this.log(`To retry: delete BOTH ${startedRel} and ${failedRel}, then re-run (or wait for the next scheduled fire). ` +
