...
export default class Next extends Command {
    static description = 'Scan the consumer repo for the next ready phase and report what would happen.';
    static examples = [
    ...
    static flags = {
        ...
        json: Flags.boolean({
            description: 'Emit a single-line JSON status object (for CI consumption). Suppresses text output.',
            default: false,
        }),
    };
    async run() {
        ...
        if (kill.active) {
            if (flags.json) {
                this.log(JSON.stringify({ kind: 'kill-switch', sources: kill.sources, details: kill.details }));
                this.exit(0);
            }
        ...
        this.log(`Note: \`next\` reports only. To fire, run \`session-orchestrator run --phase ${next.phase}\`.`);
    }
}
